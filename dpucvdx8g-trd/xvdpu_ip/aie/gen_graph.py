...
def getCoreRowCol(idx_bat, idx_ker):
    if(device==1502):    
        col_per_bat = ((NUM_PORT_B//2)*3//2 + COL_INTERVAL)
        if(idx_ker<12):
            strCol = str(int(BATCH0_COL_OFFSET +idx_bat*col_per_bat + (idx_ker//16)*2 + idx_ker%2));
            strRow = str(int(BATCH0_ROW_OFFSET +(idx_ker%16)//2));
        elif(12<=idx_ker<16):
            strCol = str(int(BATCH0_COL_OFFSET +idx_bat*col_per_bat + 2 + (idx_ker//16)*2 + idx_ker%2));
            strRow = str(int(BATCH0_ROW_OFFSET +(idx_ker%16)//2 - 6));
        elif(16<=idx_ker<28):
            strCol = str(int(BATCH0_COL_OFFSET +idx_bat*col_per_bat + 2 + (idx_ker//16)*2 + idx_ker%2));
            strRow = str(int(BATCH0_ROW_OFFSET +(idx_ker%16)//2));
        elif(28<=idx_ker<32):
            strCol = str(int(BATCH0_COL_OFFSET +idx_bat*col_per_bat + (idx_ker//16)*2 + idx_ker%2));
            strRow = str(int(BATCH0_ROW_OFFSET +(idx_ker%16)//2 - 2));
    else:
...
def genConfig(corenum, batch_num):
    ...
    if corenum == 2:
       ...
       WIDTH_PORT_C = 64
       if(device==1502):
            PLIOA_IDX2COL[0:7] = [ 4, 5, 4, 5, 0, 1, 2, 3]
            PLIOC_IDX2COL[0:15] = [1, 0, 1, 0, 1, 0, 3, 2, 5, 4, 5, 4, 5, 4, 3, 2]
       else:
...
def main(argv):
    if(len(argv) != 5):
        print("usage: ./gen_graph.py <core_num/batch> <batch num> <vver> <device>")
        sys.exit()
    corenum = int(argv[1])
    batchnum = int(argv[2])
    vver = int(argv[3])
